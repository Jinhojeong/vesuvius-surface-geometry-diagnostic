...
def main():
    imgs = sorted(glob.glob(f"{OUT}/imagesTr/*_0000.tif"))
    ...
    out["finetuned"] = report("FINE-TUNED", cF, vF)
    print("\nVERDICT: ceiling is",
          "RESOLUTION-limited (few bimodal even after FT)" if out["finetuned"]["bimodal_frac"] < 0.2
          else "partly post-processable (FT resolves sheets into 2 peaks)")
    json.dump(out, open(f"{OUT}/bimodal_taxonomy2.json", "w"), indent=1)
